...
export default function ProductItem({ name, img, content }: ProductProps) {
	return (
		<article className="w-full flex flex-col md:odd:flex-row md:even:flex-row-reverse items-center justify-center gap-0.5 group">
			<div className="w-full md:w-1/2 min-h-110 md:min-h-142.5 p-3 sm:p-6 rounded-tl-[50px] rounded-br-[50px] flex flex-col items-center justify-evenly gap-3 text-center text-white group-odd:bg-primary group-even:bg-secondary">
				<h5 className="font-poppins font-bold text-center text-2xl md:text-3xl capitalize">
					{name}
				</h5>
				<p className="text-xs sm:text-base text-center text-dark-grey md:w-4/5">
					{content}
				</p>
				<Link
